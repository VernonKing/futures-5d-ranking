...
`.github/workflows/deploy.yml`在工作日北京时间16:00首次触发，并在16:25补跑。新浪日线可能分批发布，抓取器会重试日期落后的品种；补跑用于在首次数据仍不完整时继续更新。GitHub定时任务可能排队延迟，因此不保证分秒不差；无人访问时仍会更新，也不依赖用户电脑或Codex运行。
