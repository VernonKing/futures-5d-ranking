...
公开静态行情网站，按国内期货连续合约最近5个完整交易日的收益率进行分类排名。
...
`.github/workflows/deploy.yml`在工作日北京时间15:40首次触发，并在16:20、17:20补跑。新浪日线可能分批发布，抓取器会重试日期落后的品种；补跑用于在首次数据仍不完整时继续更新。GitHub定时任务可能排队延迟，因此不保证分秒不差；无人访问时仍会更新，也不依赖用户电脑或Codex运行。
...
行情为新浪财经国内期货连续合约公开数据，可能存在延迟、缺失或连续合约调整。页面仅用于趋势观察，不构成投资建议。
